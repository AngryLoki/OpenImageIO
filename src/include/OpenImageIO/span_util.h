...
///
/// The solution is to write a "make_span" function that will deduce the
/// template type and return the correct span<T>. So we do so for the
/// particular cases of std::vector<T> and T[N] arrays. This is not a complete
/// solution, but it's enough for our purposes.

// Helpers: make spans out of common containers
template<typename T, class Allocator>
inline constexpr span<T>
make_span(std::vector<T, Allocator>& arg)
{
    return { arg };
}
...
make_cspan(const std::vector<T, Allocator>& arg)
{
    return { arg };
}
...
/// (which should be `n` if the operation was fully successful, but may be
/// less if the request could not be satisfied while staying within the span
/// bounds).
///
